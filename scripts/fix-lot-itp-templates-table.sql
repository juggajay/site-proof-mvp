-- Fix lot_itp_templates table to use correct data types
-- This script changes itp_template_id from UUID to INTEGER to match itp_templates.id

-- First, drop the existing table (if it has no important data)
-- WARNING: This will delete all existing assignments!
...
CREATE TABLE lot_itp_templates (
    ...
    itp_template_id INTEGER NOT NULL REFERENCES itp_templates(id) ON DELETE CASCADE,  -- Changed from UUID to INTEGER
